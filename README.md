...
В разделе **«Информация»** есть кнопки **«Собрать отчёт»** и **«Проверить устройство»**. Отчёт копирует версию приложения, Android SDK, ABI и разрядность, размер страницы памяти, модель устройства, SoC, ROM, память, состояние сети, WebView и безопасные параметры запуска. Проверка устройства отдельно показывает текущую версию WDTT Plus, дату релиза, актуальна ли версия на момент копирования, совместимость, предупреждения, текущее состояние VPN и переходы в нужные настройки Android. Адреса, пароли, токены и значения VK-хешей в отчёт не включаются.
