...
**[Официальный Telegram-бот WDTT Plus](https://t.me/WDTTPlusBot)** — позволяет получить готовый профиль и открыть его в приложении.

## Инструменты в репозитории

- **[Ручной установщик сервера WDTT Plus](server-installer/README.md)** — самостоятельный Linux-скрипт для безопасной установки, обновления, диагностики и обслуживания `wdtt-server` без передачи SSH-реквизитов Android-приложению. Поддерживает проверку совместимости, резервирование данных и откат при ошибке.
- **[Менеджер выборочного WARP](warp-interface-manager/README.md)** — отдельный самостоятельный проект для направления IPv4-трафика с выбранного Linux-интерфейса через Cloudflare WARP без замены основного маршрута сервера.
...
- **Обновление, резервирование и перенос сервера:** повторный деплой предлагает безопасное обновление с сохранением данных или чистую установку. Сервер умеет создавать проверяемые копии по расписанию; приложение показывает историю, позволяет создать, проверить, скачать, загрузить, восстановить или удалить копию. Перед обновлением и восстановлением создаются постоянная и транзакционная страховочные копии, а при ошибке выполняется откат.
...
- **Прокси без VPN-слота:** профиль может поднять SOCKS5, HTTP CONNECT или автоматический совместный обработчик на одном порту вместо системного VPN. SOCKS5 поддерживает TCP `CONNECT` и UDP `ASSOCIATE`, а HTTP CONNECT добавляет совместимость с TCP-программами без SOCKS5. Доступ можно оставить на `127.0.0.1`, безопасно открыть для ближайшей локальной подсети либо включить оба точных адреса одновременно; любой LAN-доступ требует аутентификацию.
...
- **SOCKS5-режим:** userspace WireGuard поднимает SOCKS5, не запрашивает VPN-разрешение и не занимает системный VPN-слот. Для цепочки внешний VPN-клиент должен исключить пакет `com.wdtt.plus`, использовать SOCKS5 как следующий узел и запретить прямой fallback. Поддерживаются TCP и обычные UDP-датаграммы; IPv6 и фрагментация SOCKS5 UDP не выдаются за поддерживаемые.
- **HTTP CONNECT:** отдельный режим TCP-прокси для браузеров и программ без SOCKS5.
- **Автоматический прокси:** SOCKS5 и HTTP CONNECT работают на одном номере порта, а каждое TCP-соединение направляется по однозначному приветствию клиента. SOCKS5 использует TCP и UDP, HTTP — TCP. По умолчанию прокси готовит доступ и для приложений телефона, и для текущей частной локальной сети; слушатели привязаны к точным IPv4-адресам, общий лимит клиентов не умножается, а LAN-доступ не включается без логина и пароля.
...
- **Резервные копии сервера:** во вкладке **«Деплой»** настраиваются автоматический интервал и количество хранимых копий, отображаются последняя попытка, следующая копия и история. Выбранную копию можно проверить, восстановить, скачать на устройство в зашифрованном виде или загрузить обратно через системный выбор файла Android.
...
```text
VPN:    Android → VpnService / WireGuard GoBackend → локальный UDP 127.0.0.1:9000
SOCKS5: приложение цепочек → 127.0.0.1:1080 → userspace WireGuard → тот же локальный UDP
                   → Go-клиент WDTT → WRAP RTP AEAD → VK TURN / DTLS → wdtt-server → интернет
```
...
6. В режиме VPN Android-часть поднимает системный WireGuard и применяет исключения приложений. В режиме SOCKS5 тот же конфиг запускается внутри userspace netstack, а наружу открывается только локальный прокси.
...
Для переноса на другой сервер во вкладке **«Деплой»** используйте блок **«Резервные копии и перенос»**. Частичный экспорт сохраняет базу WDTT Plus: профиль владельца, Telegram-настройки, клиентов, привязки устройств, статистику и историю. Полный экспорт дополнительно требует и сохраняет WireGuard-ключи сервера; формат v3 также переносит сохранённый профиль выходного IP, прокси и VPN/WireGuard. Старые `wdtt://` ссылки останутся рабочими без перевыдачи, если в них был домен, DNS указывает на новый сервер и сохранены прежние порты; для ссылок со старым IP или при смене портов создайте новые. Импорт можно выполнить при деплое или на работающем сервере: режим **«Заменить»** подходит для переезда, режим **«Добавить»** переносит отсутствующие записи, проверяет конфликты устройств и назначает свободные внутренние IP. Перед изменением создаются постоянная и транзакционная страховочные копии, после импорта проверяются данные и служба, а при ошибке выполняется откат.
...
Если вы не хотите передавать административные SSH-реквизиты приложению для
первой установки, используйте автономный Linux-установщик из каталога
[`server-installer`](server-installer/README.md). После ручной установки WDTT
Plus подключается по адресу, DTLS-порту и паролю туннеля; SSH нужен только для
деплоя, администрирования и управления резервными копиями, но не для обычного
VPN-подключения.

Ручной установщик не входит в APK и не запускается приложением. Android
распознаёт принадлежащую ему установку и блокирует поверх неё обновление,
повторный деплой, сброс и удаление. Новый бинарник и изменения портов применяются
самим `server-installer/install.sh`; подключение, управление клиентами и
резервными копиями, включая восстановление под общей блокировкой, остаются в
приложении.
...
Управление находится во вкладке **«Деплой» → «Резервные копии и перенос»**.
Расписание выполняется внутри `wdtt-server`, поэтому приложение можно закрыть.
Можно выбрать интервал от 6 до 168 часов и хранить от 2 до 90 исправных копий.
При включении расписания сервер сначала создаёт и перечитывает первую копию;
если это не удалось, настройка не включается. Просроченная копия создаётся после
возвращения службы в рабочее состояние, а неудачные попытки повторяются с
ограниченной задержкой.

Каждый серверный снимок содержит:

- `passwords.json`: главный профиль, настройки сервера, клиенты, привязки,
  сроки, статистику и историю;
- `wg-keys.dat`: серверную идентичность WireGuard;
- сохранённый профиль выхода `outbound-profile.env`, если он был настроен.

Фактические внешние интерфейсы, работа второго VPS и сторонние прокси таким
снимком не создаются: после переезда их доступность нужно проверить отдельно.
Копии на VPS хранятся только в закрытом для root каталоге и не защищают от
потери всего VPS. Для внешней копии выберите **«Скачать»**, задайте отдельный
пароль и сохраните файл через системный выбор места Android. Приложение не
сохраняет этот пароль. Файл можно позднее выбрать через импорт и восстановить
на текущий или другой совместимый сервер.

Перед восстановлением сервер создаёт ещё одну постоянную копию текущего
состояния, затем приложение готовит отдельный транзакционный откат. Замена и
удаление запускаются только после непрерывного удержания опасной кнопки в
течение трёх секунд; отпускание или потеря связи сразу отменяет отсчёт. Сервер
не удаляет
последнюю исправную копию, а повреждённые файлы отмечает в истории и не удаляет
самостоятельно. Старые незашифрованные переносимые файлы v1/v2 читаются для
совместимости, но новые выгрузки всегда защищаются паролем.
...
Перед загрузкой выпуска на GitHub используйте отдельную fail-closed подготовку:

```bash
./gradlew :app:prepareGithubReleaseArtifacts
```

Она сначала удаляет прежний готовый набор, выполняет release-сборку и закрытый аудит, а затем создаёт `app/build/outputs/release-ready/` ровно с шестью файлами: четырьмя APK, standalone-архивом того же серверного бинарника и его внешней контрольной суммой. Если сервер внутри хотя бы одного APK, standalone-архива или текущих исходников отличается, готовый каталог не создаётся. Самостоятельный WARP-менеджер остаётся отдельным подпроектом и в выпуск WDTT Plus автоматически не упаковывается.
