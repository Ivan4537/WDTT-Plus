...
- **Расширенное управление сервером:** клиентов и основные параметры сервера можно обслуживать прямо из приложения без Telegram. Telegram-бот остаётся альтернативным удалённым пультом с теми же основными операциями.
...
- **Ссылки, роли и перенос между устройствами:** подключение можно передать ссылкой, файлом или QR-кодом, а получить — также камерой и из изображения. Импорт проверяется и занимает свободный VPN-профиль; при трёх занятых приложение запрашивает выбор профиля для замены. Для администратора доступен защищённый паролем перенос локальных настроек.
...
- **Парольная модель доступа:** сервер поддерживает главный пароль, настраиваемый лимит пользовательских паролей, 16-символьную генерацию, привязку к устройству и срок действия. Клиентов можно создавать, отключать, продлевать, отвязывать и удалять из приложения или Telegram-бота.
...
В блоке **«Клиенты и сервер»** вкладки **«Деплой»** можно искать и фильтровать клиентов, создавать доступы, редактировать их параметры, срок и привязку устройства, а также передавать подключение ссылкой, файлом или QR-кодом. Там же доступны история, трафик, настройки сервера, очистка устаревших записей и сервисные действия.
